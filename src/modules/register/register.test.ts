...
describe("Register resolver", () => {
  ...
  it("should catch a bad email", async () => {
    const response = await request(host, mutation(BAD_EMAIL, PASSWORD));
    expect(response.register).toHaveLength(1);
    expect(response.register[0]).toEqual({
      path: "email",
  ...
  it("should catch a short password", async () => {
    const response = await request(host, mutation(EMAIL, SHORT_PASSWORD));
    expect(response.register).toHaveLength(1);
    expect(response.register[0]).toEqual({
      path: "password",
  ...
  it("should catch a long password", async () => {
    const response = await request(host, mutation(EMAIL, LONG_PASSWORD));
    expect(response.register).toHaveLength(1);
    expect(response.register[0]).toEqual({
      path: "password",
  ...
  it("should catch both invalid inputs", async () => {
    const response = await request(host, mutation(BAD_EMAIL, SHORT_PASSWORD));
